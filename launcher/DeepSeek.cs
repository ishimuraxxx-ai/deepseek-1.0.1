...
// Расширение (extension\) вшито в DeepSeek.exe, поэтому для установки достаточно одного файла:
// скачанный exe копирует себя в %LOCALAPPDATA%\Programs\DeepSeek, распаковывает туда расширение
// и кладёт ярлыки DeepSeek на рабочий стол и в меню «Пуск».
...
static class Launcher
{
    ...
    static void Main()
    {
        ...
        if (!portable)
        {
            dir = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), @"Programs\DeepSeek");
            exe = Install(exe, dir);
        }
    ...
    static string Install(string exe, string dir)
    {
        ...
        string target = Path.Combine(dir, "DeepSeek.exe");
    ...
    // Кладёт в папку folder ярлык DeepSeek.lnk на exe.
    ...
    static void EnsureShortcut(string folder, string exe, string dir)
    {
        try
        {
            string path = Path.Combine(folder, "DeepSeek.lnk");
